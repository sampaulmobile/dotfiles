...
- NEVER run `tmux kill-server`, `kill-session`, `source-file`, `set-hook`, or
  ...
  killing it destroys the user's whole working state.
- Tests needing a tmux server MUST use a throwaway socket with the `-L <name>`
  ...
  (python subprocess env, etc.) is FORBIDDEN — indirection hides which server
  the command will reach, and the wrong answer is the live one.
