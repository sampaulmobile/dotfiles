...
- Find a checkout of the branch: `git worktree list`, use the worktree that has it checked out; otherwise create one from the hub with `wt switch <branch>` (worktrunk lands the `<repo-dir>.<branch-slug>` sibling; never raw `git worktree add`) and work there, anchoring every command on its path.
