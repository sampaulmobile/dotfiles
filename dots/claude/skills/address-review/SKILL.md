...
- Before the first commit, confirm the PR is still open (`gh pr view <n> --json state`) — it can merge mid-run, and git gives no signal. A merged PR takes no further commits: branch fresh off the default branch, carry the remaining fixes there, and say so in the report instead of committing into the merged branch.
