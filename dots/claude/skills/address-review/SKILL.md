...
- Find a checkout of the branch: `git worktree list`, use the worktree that has it checked out; otherwise create one with `wt switch <branch>` run from the repo's MAIN checkout — `dirname $(git rev-parse --git-common-dir)`, or the repo path in the hq routing table (worktrunk lands the `<repo-dir>.<branch-slug>` sibling; never raw `git worktree add`) — and work there, anchoring every command on its path.
...
- Maintain `.feature/status.jsonl` in the worktree (create `.feature/` and exclude it via `$(git rev-parse --git-common-dir)/info/exclude` if absent) by APPENDING lines, never rewriting one: `{"repo","branch","pr","phase":"review-fix","round":<N>,"seat":"implementer","task":"<thread being addressed>","last_commit","pushed","updated":<ISO-8601 UTC>,"started_by":"user:/address-review" or "hq:<session>"}` — one on start, one on each commit, one with `phase: done` after the push. Its LAST line is the contract a coordinator (hq) and `bin/worktrees` read to see where the pipeline is without a transcript.
...
- Before the first commit, confirm the PR is still open (`gh pr view <n> --json state`) — it can merge mid-run, and git gives no signal. A merged PR takes no further commits: branch fresh off the default branch in its own worktree (`wt switch -c <branch>` run from the MAIN checkout, as above), carry the remaining fixes there, and say so in the report instead of committing into the merged branch.
- Commit specific files with clear messages (never `git add -A`). The commit body is where the what/why of each fix lives — say which feedback it addresses.
