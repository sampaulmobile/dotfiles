...
Spawn a FRESH reviewer subagent (`subagent_type: "<REVIEWER_EFFORT>"`). Model: `<REVIEWER_MODEL>` in round 1; `opus` in every later round and in any prose-only round; `<REVIEWER_MODEL>` in every round when <REVIEWER_PINNED> is true. Its prompt must include this severity bar verbatim (unless <STRICT> is true, in which case: any finding with a concrete failure scenario is blocking): "Mark a finding BLOCKING only if a senior human reviewer would request changes for it — a concrete correctness bug, security issue, missing test for new behavior, or an unintentional workplan deviation. Hardening suggestions, docs polish, and style preferences are non-blocking notes. Do not manufacture blockers to justify the round: on a small clean diff, CLEAN is the correct and expected verdict. An input that cannot occur given its trust boundary — operator-set config or an internal caller, not untrusted/network input — is a non-blocking note at most, never a blocking finding; do not require hardening against unrealizable inputs." The prompt must also tell it:
- Coverage. Round 1 is a FULL pass over DELTA (the whole PR). Round N>1 is a SCOPED pass: DELTA plus the functions it touches, their callers and their tests, and a check that every blocking finding in `.feature/findings-round-<N-1>.md` is addressed. Escalate a scoped pass to a full one, and say so on the second line of the findings file, when DELTA touches a shared function, a hook or hook config, or anything on a destructive, network-facing or credential-adjacent path.
- Code review, when the round is not prose-only: invoke the `code-review` skill on DELTA — that IS the code review; do not run a second independent review of the same diff. If the skill is unavailable, review DELTA directly for correctness bugs, unhandled edge cases, missing or weak tests and security issues. Then add only what the skill does not know: conformance to the workplan (scope creep, silently-skipped plan items; check `.feature/NOTES.md` before calling a deviation unintentional), and conformance to the repo's CLAUDE.md and the global rules — comments, public-repo hygiene, deletion safety, tmux safety, CI/CD safety — with comment/prose noise as non-blocking notes and deletion of a load-bearing guard/why comment treated as a code change, not cleanup.
- Prose-only round: no `code-review`, no probes. Check: contradictions between the changed files and the files they describe, stale file/flag/path references, public-repo leaks, comments-rule violations, and flag lists that match the flags the skill defines. Blocking only for a leak or a contradiction.
- Verification check, code rounds only: did the implementer name the surface each change's consumer meets and observe the change there, or say "inconclusive"? A claim with no command and no output behind it is a finding; so is a surface nobody checked. Run the check yourself where it is cheap and read-only.
- Blast radius, only when DELTA adds or changes something destructive, network-facing or credential-adjacent (round 1), or touches such code (later rounds): name the ONE fact the change is safe because of and prove it with read-only commands and throwaway inputs. Never execute a deletion as a probe, whatever the target is redirected to — the global deletion rule applies; a deletion that cannot be proven safe by reading is a finding that asks for a redesign. Unproven is reported as unproven, not as safe.
- Write `.feature/findings-round-<N>.md`: line 1 the verdict, `CLEAN` or `FINDINGS`; line 2 the pass, `full`, `scoped`, `scoped→full: <why>` or `prose`; then blocking findings (file:line, what is wrong, concrete failure scenario), then non-blocking notes, then the blast-radius line when one was required. Nothing else — no narrative of what was checked.
