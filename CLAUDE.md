...
- `bin/` — scripts (install helpers, tmux scripts, worktree helpers). Two read-only doctors live here; both change nothing and exit 1 when they flag something:
  ...
  - `bin/worktree-doctor` — every worktree of every project `bin/project-dirs-lib` knows about, agent worktrees included, bucketed LIVE / DIRTY / PRUNABLE / MERGED / ABANDONED / IN-REVIEW / PARKED / SHIPPED? (first match wins) with the command that would act on it printed verbatim: `wt` and `git worktree prune` are shown, never run. Liveness from `lsof -d cwd -Fpn`, never the worktree lock file; PR state from `gh`, with `--offline` degrading every row to `pr:unknown`. Exit 1 while anything is MERGED/ABANDONED/PRUNABLE; `--all` also lists repos with nothing extra.
...
- `tests/` — one offline suite per pure function in the agent layer (the pane-status classifiers, the hook payload schemas, the codex rollout reader, the agent-per-tty parse, the sessionizer's fzf `--expect` parse, the pipe-truncate guard, the settings-drift check, `bin/project-dirs-lib`'s enumeration/naming, `bin/worktree-doctor`'s buckets) plus fixtures. `tests/run.sh` runs everything under both `/bin/bash` (3.2) and the default bash. No tmux server is started or touched. `tests/check-prose-only.sh <base-ref>` is the tripwire for comment-only passes: it fails if a changed `bin/` or `tests/` file differs once full-line comments and blank lines are stripped.
...
- `other/` — machine-local config, gitignored except `*.example` templates and README. Consumed via: zshrc sources `other/zshrc.local` last; gitconfig includes `other/gitconfig.local`; `bin/project-dirs-lib` sources `other/tmux-sessionizer.local` (can redefine `search_dirs`); `other/claude/` symlinked into `~/.claude/` by `bin/symlink_files.sh` (settings.json file link; `skills/`, `rules/`, `agents/` dir links — anything dropped into `~/.claude/{skills,rules,agents}` therefore lands here, private by default); `other/codex/` the same way for codex (config.toml, AGENTS.md, skills/); `other/tmux-agent-default` picks the default agent for new sessions
...
- `bin/tmux-sessionizer` — Ctrl+F's picker, over the shared `project_dirs` enumeration plus every live session. INVARIANT: nothing is scanned at keypress time — rows stream straight into fzf and status colors come from caches (the statusbar's `status.tsv`, ignored when stale, plus the attention `.done` flags). Red = permission prompt waiting, green = finished-but-unseen, blue = working, dim blue = idle agent; a dir with no session yet is dim with a `~`-shortened path, cyan-tinted when it is a linked worktree (`.git` is a file). Worktree rows read `<~repo-path>  <origin>  ⎇ <branch>`, where the origin tag (`agent` = harness-made under `.claude/worktrees/`, `wt` = worktrunk sibling) doubles as a filter word and a sibling shows its repo's path, not its own `proj.feat-x` dir name. `ctrl-/` toggles a lazy preview (pane snapshot for sessions, git status/log for dirs). `Enter` builds a new session with the default agent, `ctrl-x` or Option+Enter with the other one (fzf `--expect` keys, not tmux bindings; the header line names both); on a row that already has a session all three just switch to it.
- `bin/project-dirs-lib` — the one source of truth for "which directories are projects" and what their sessions are called; fork-free and sourceable. Owns `search_dirs` (the "path:depth" list — `~/dev` at depth 1, `~/dotfiles` at depth 0) and its `other/tmux-sessionizer.local` override; `project_dirs`, which also enumerates legacy `.bare/` containers' worktrees, agent worktrees (`<repo>/.claude/worktrees/agent-<id>`, made by the Agent tool and by `/feature`) and worktrunk siblings (`~/dev/proj.feat-x`, `~/dotfiles.feat-x`); and the naming rule in `session_name_for` / `worktree_branch` / `worktree_repo_out` — `<project>_<branch>` for a worktree with branch slashes sanitized to `-` to match worktrunk (so an agent worktree and a `wt switch -c` sibling for one branch share a session), `<basename>` otherwise, dots → underscores. Sourced by `bin/tmux-sessionizer`, `bin/worktree-doctor`, `bin/wt-tmux-jump` and `bin/wt-tmux-cleanup`, so `wt remove` on an agent worktree kills the session Ctrl+F created for it. `session_name_for_branch <repo-name> <branch>` is the same rule with no filesystem read, used via `session_name_for_removed` by the post-remove/post-merge hooks, which run after the worktree directory (and its `.git` file) is gone. See `tests/test-project-dirs.sh`.
...
- `bin/tmux-claude-dashboard` — Ctrl+G's interactive dashboard for all claude and codex sessions. One row per agent PANE, from `get_claude_panes` (whose 4th column names the row's agent), so an agent outside window 1 gets its own `session:N` row (N = window index) with its own transcript, tokens and subagent tree. Columns SESSION, STATUS, MODEL, CONTEXT, CTX%, TURNS, OUTPUT, TOTAL, COST fill the terminal; SESSION absorbs the spare width, the rest pin right, and columns drop before names truncate.
  - CONTEXT is used/total of the model's window (`162.7k/1M`, sized by `context_window_for`: 1M for the current Fable/Opus/Sonnet lineup, 200K Haiku/older, bare number when unknown). CTX% is the same as a percent and drives the coloring — yellow ≥50%, red ≥70%, bold red ≥90%, absolute thresholds when the model is unknown.
  - TURNS = user prompts handled, not API requests. COST is `$1.23` exact (claude's own cost-state snapshot, subagents included — live sessions rarely have one), `~$1.23` estimated over the main transcript only, or `-`. That price table is duplicated from claude-tokens' JQ_PRELUDE: keep the two in sync. Sums come from `get_totals_incremental` (requestId-deduped).
  - Idle, done and permission rows carry an age from the transcript's last write (`⏳ perm 9m`); a session whose CWD is gone shows `✗ gone`. Keys: `j/k` navigate, `⏎` switch to the row's exact pane, `x` kill row (the session when it is that session's only agent row, else just the pane), `X` prune gone rows, `a` toggle agent trees, `r` refresh, `R` hard refresh (clears the token cache), `q` quit.
  - A session row expands into its live subagents, indented by spawn depth, parents before children, on the same grid: `working 12m` counts from spawn, `idle 3m` from the last write, MODEL is `agentType·model` — so a per-seat override (`high·opus` implementers vs `high·fable` reviewers) is visible instead of every `high` row reading alike, with the `·model` half dropped when unknown. A working agent's snippet prints as a dim quoted line beneath it: latest narration, else the last tool call as `→ Tool: arg` (`get_agent_snippet`, `tail -c 131072`). A `+N` badge marks sessions with N agents working; those auto-expand, `a` forces every tree open.
  - Transcripts resolve pane-exactly via the `tmux` field in `~/.claude/sessions/*.json` (`find_pane_jsonl`), so two claudes sharing a cwd cannot read each other's. Codex rows differ three ways: window from the rollout's own `model_context_window`, COST as the plan's rate-limit usage (`5h:42%`), no subagent tree.
- `bin/tmux-claude-statusbar` — ambient attention layer in the status bar, refreshed every ~5s: `🔴N ✅N ●N ` for permission-needed / finished-but-unseen / working, each segment omitted at zero and the whole thing empty when all are. 🔴 and ● count sessions, ✅ counts flagged panes. The scan is `scan_claude_sessions` in `bin/tmux-claude-lib`, shared with Ctrl+Y. Each tick also publishes `~/.cache/claude-attention/status.tsv` (`<session>\t<status>` plus the pane exhibiting it, written atomically) — the sessionizer's row colors read that.
- `bin/tmux-claude-attention` — flag-file backend for the attention layer: `~/.cache/claude-attention/<session>@<pane>.done`, one per finished-but-unseen agent PANE. Everything is per pane, never per session: a session-level flag both suppresses an agent finishing in a background window of an attached session and clears a window nobody looked at. The same dir holds the statusbar's `status.tsv` and `codex/<pane>.tsv`, the pane → rollout map codex's SessionStart hook writes (codex records no tmux pane of its own); `codex-session-end` drops an entry and `list` prunes panes that are gone. Subcommands: `flag-done` (the Claude Code Stop hook — the script's header has the settings.json snippet) flags `$TMUX_PANE` unless that pane is on screen; `clear <session>` clears the panes of the session's *current window* only, fired by the client-attached, client-session-changed and session-window-changed tmux hooks; `list` prints flagged live panes newest-first; `jump` is Ctrl+Y's backend. `clear` gates on an attached client, because session-window-changed also fires when a script select-windows in a detached session.
...
- `bin/claude-tokens` — offline token/cost analyzer over the `~/.claude/projects` transcripts; claude only, no tmux, and it full-parses every file it reports on, so expect seconds. Grains: bare `claude-tokens`, `sessions [fragment]`, `session <id|path>`, `pr <num|url-frag>`, `branch <name>`, with `--since 7d/24h/90m` windowing all but `session` and `pr`; `--help` (the script's own header) has the details. Cost is claude's own cumulative figure where cost-state records exist, subagents included, else a price-sheet estimate prefixed `~` (~10% under actual: retries and utility calls never reach the transcript). Two transcript facts drive the tool: assistant records are deduped per requestId, and a resume copies the whole history into a new file (per-record snake_case `session_id` keeps the original id, camelCase `sessionId` is rewritten), so listings collapse each chain into one row keyed by the live tip and mark an ancestor-derived cost `>`.
