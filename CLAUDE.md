...
- `bin/` — scripts (install helpers, tmux scripts, worktree helpers). Two read-only doctors live here; both change nothing and exit 1 when they flag something:
  ...
  - `bin/worktree-sweep` — the actor on the doctor's report: removes only the MERGED bucket, and only after a gitignored-data guard clears each one. It SOURCES `bin/worktree-doctor` for `parse_porcelain_blocks` / `derive_worktree_facts` / `classify_worktree`, so "merged" (squash merges included, via `gh`) and the LIVE/DIRTY/PRUNABLE protections are identical to the report — never a second copy. Dry-run by default; `--apply` removes (via `wt -C <repo> remove --foreground`, no `-f`), `--force` also removes MERGED worktrees the guard skipped, `--offline` drops `gh`. The guard: a merged worktree's commits are safe on the default branch, but `git worktree remove` deletes its GITIGNORED files silently — so a file byte-identical to the hub's copy (copy-ignored seeded it) or a regenerable cache (the copy-ignored exclude list, read live from `dots/worktrunk.toml` — one source of truth) is safe to drop; anything else (a fresh `.env`, a populated DB, `.feature/` state, any ignored directory) is worktree-unique data → SKIP and list it. Exit 2 on a removal failure under `--apply`.
...
- `bin/tmux-claude-statusbar` — ambient attention layer in the status bar, refreshed every ~5s; the glyph legend is the script's header. The scan is `scan_claude_sessions` in `bin/tmux-claude-lib`, shared with Ctrl+Y. Each tick also publishes `~/.cache/claude-attention/status.tsv` (`<session>\t<status>\t<pane>`, written atomically) — the sessionizer's row colors read that instead of scanning.
