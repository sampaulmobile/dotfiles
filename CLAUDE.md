...
- `Ctrl+Space` — toggle claude per project: jump to/from the `claude` window inside the project session, creating it on first use (`bin/tmux-claude-window`)
- `prefix+C` — extra claude window in the current session: new window at the caller pane's directory (repo dir in a hub/worktree session, wherever you've cd'd otherwise) running claude via a shell. Named `claude+`, not `claude`, so the C-Space by-name lookup never grabs it; dashboard/statusbar/Ctrl+Y track claudes by tty, so extra windows show up in all of them automatically (the dashboard gives each its own `session:N` row). Shadows tmux's default customize-mode binding.
