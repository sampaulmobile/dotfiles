...
- `bin/tmux-claude-lib` — shared functions sourced by the claude session manager scripts; home of `new_hub_session` (the standard hub session layout, used by the sessionizer and `wt-tmux-jump`) and `scan_claude_sessions` (the multi-pane status scan behind the statusbar and Ctrl+O).
