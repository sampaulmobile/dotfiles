...
- `/brainstorm <idea>` is the tier above `/feature`: a Socratic, one-question-at-a-time design session for meaty or greenfield work, ending in a durable design doc (not a workplan). Two modes by cwd: repo mode (a repo hub) writes to `<repo>/docs/design/<slug>/`; greenfield mode (anywhere else, incl. hq) writes to `~/dev/hq/designs/<slug>/`. It stops short of writing workplans, creating repos, or invoking `/feature` — that flow starts from the finished design doc. Claude-only (needs `AskUserQuestion` and the Agent tool).
