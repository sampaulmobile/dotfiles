...
- `bin/` — scripts (install helpers, tmux scripts, worktree helpers). Two read-only doctors and the one sweeper that acts on their output live here; the doctors change nothing and exit 1 when they flag something:
  ...
  - `bin/worktree-sweep` — the actor on the doctor's MERGED bucket, sourcing the doctor's own classification (never a second copy) so the two can never disagree on "merged". Dry-run unless `--apply`; even then it only ever removes a worktree the doctor buckets MERGED, and only past a gitignored-data guard (a file must be a regenerable cache or byte-identical to the hub's copy to count as safe — anything else, including an ignored directory that doesn't reduce to one of those, SKIPs the worktree and lists what's at risk; `--force` overrides the skip). Exit 0 on a clean run, 2 on a removal failure under `--apply`; flags are in `--help`.
