...
- `bin/` — scripts (install helpers, tmux scripts, worktree helpers). Two doctors live here and exit 1 when they flag something; `bin/doctor` is read-only, `bin/worktrees` is read-only except under its `sweep --apply` verb:
  ...
  - `bin/worktrees` — every worktree of every project `bin/project-dirs-lib` knows about, agent worktrees included, bucketed LIVE / DIRTY / PRUNABLE / MERGED / ABANDONED / IN-REVIEW / PARKED / SHIPPED? (first match wins), as one aligned table grouped act / surface / in-progress; `-v` adds the command that would act on each row. Liveness from `lsof -d cwd -Fpn`, never the worktree lock file; PR state from `gh`, with `--offline` degrading every row to `pr:unknown`. Exit 1 while anything is MERGED/ABANDONED/PRUNABLE; `--all` also lists repos with nothing extra. `worktrees sweep` is the actor on the MERGED bucket — same program, same classification, so the two can never disagree on "merged". Dry-run unless `--apply`; even then it only removes a worktree bucketed MERGED that sits at the merged PR's head (or has nothing ahead of `origin/<branch>`), and only past a gitignored-data guard: a file must be a regenerable cache (the copy-ignored exclude list) or byte-identical to the hub's copy to count as safe — anything else SKIPs the worktree and lists what's at risk; `--force` overrides the skip. Exit 0 on a clean run, 2 on a removal failure under `--apply`; flags are in `--help`.
