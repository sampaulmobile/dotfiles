...
- `tests/` — offline bash tests for the agent layer (pane-status classifier, notify hook schemas, codex rollout reader, the agent-per-tty parse, the sessionizer's fzf `--expect` parse) plus their fixtures. `tests/run.sh` runs everything under both `/bin/bash` (3.2) and the default bash. No tmux server is started or touched.
