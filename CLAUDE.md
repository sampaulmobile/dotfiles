...
- `bin/` — scripts (install helpers, tmux scripts, worktree helpers). `bin/profile-lib` is the shared opt-in stage timer for the three scripts whose speed is a feature: `WORKTREES_PROFILE=1`, `PRS_PROFILE=1`, `DASHBOARD_PROFILE=1` each print one stderr line per stage, and cost nothing when unset. Two doctors live here and exit 1 when they flag something; `bin/doctor` is read-only, `bin/worktrees` is read-only except under its `sweep --apply` verb:
